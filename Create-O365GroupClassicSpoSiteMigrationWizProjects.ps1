...
3. Change source and/or destination MigrationWiz migrations after cutover `
4. Delete created MigrationWiz endpoints and projects
-----------------------------------------------------------------------------------------------------------------------
5. Exit
...
write-host 
do {
    if(!$script:createMigrationWizProjects -and !$script:ApplyBitTitanLicenses -and !$script:changeMigrations) {
...
while (!$migrateSpoTeamSites -and !$migrateO365Groups)

if($script:createMigrationWizProjects -or $script:ApplyBitTitanLicenses -or $script:changeMigrations) {
write-host 
$msg = "#######################################################################################################################`
                       PROJECT SCOPE               `
#######################################################################################################################"
...
#######################################################################################################################"
    ...
    if($migrateSpoTeamSites) {
        Get-SourceTenantAssessment -migrateSpoTeamSites $migrateSpoTeamSites
    } 
    
    if($migrateO365Groups) {
        Get-SourceTenantAssessment -migrateO365Groups $migrateO365Groups
    } 
...
#######################################################################################################################"
    ...
    if($migrateO365Groups -eq $true) {
        $msg = "ACTION: User Migration Bundle licenses required (for OneDrive For Business and/or Office 365 Groups): $script:unifiedGroupsCount (without data transfer limit)."
        Write-Host -ForegroundColor Yellow $msg
        Log-Write -Message $msg 
    }
}
...
#######################################################################################################################
if($script:ApplyBitTitanLicenses -or $script:changeMigrations) {
    write-host 
$msg = "#######################################################################################################################`
...
#######################################################################################################################"
    ...
    if($script:importedClassicTeamSites -eq $null -or $script:importedDocumentLibraries -eq $null) {
        ...
        foreach ($classicTeamSite in $script:importedClassicTeamSites) {
            ...
            if($subsiteDocumentLibraries -ne $null) {
                foreach($subsiteDocumentLibrary in $subsiteDocumentLibraries  ) {
                    ...
                    if($ProcessedSubsites -ne $subsiteDocumentLibrary.SubsiteNumber.SubString(0,1)) {
                        $ProcessedSubsites += 1 
                    }  
        ...
        if($processedClassicTeamSites -ne 0) {
            ...
            write-Host -ForegroundColor Green $msg
            Log-Write -Message $msg 
            $msg = "SUCCESS: $processedSubsites out of $script:subsitesCount classic Team Subsites have been processed." 
            write-Host -ForegroundColor Green $msg
            Log-Write -Message $msg             
    ...
    elseif($script:importedUnifiedGroupDocumentLibraries -eq $null) {
        ...
        foreach ($unifiedGroup in $script:importedUnifiedGroups) {
            ...
            if($groupName -eq "" -or $srcUnifiedGroupUrl -eq "" -or $dstUnifiedGroupUrl -eq "" -or $srcMigrationWizAddress  -eq "" -or $dstMigrationWizAddress -eq "") {
                ...
                if($srcMigrationWizAddress -ne "" -and $dstMigrationWizAddress -ne "" ) { #-and $ProcessedO365GroupSites -ge 1
            ...
            if($mailboxConnectorId -ne $null) {
                ...
                if(!$mailbox) {
                    ...
                    if($mspcUser -and !$isUmbApplied -and $script:ApplyBitTitanLicenses -and !$script:changeMigrations){
        ...
        if($ProcessedO365GroupSites -ne 0) {
            ...
            Log-Write -Message $msg 
            write-Host
            $msg = "SUCCESS: $ProcessedSubsites out of $script:unifiedGroupSubsitesCount Office 365 Groups Subsites have been processed." 
            write-Host -ForegroundColor Green $msg
            Log-Write -Message $msg 
