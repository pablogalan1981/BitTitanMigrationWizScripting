...
.SYNOPSIS
    Script to add automatically all UserMapping's to a MigrationWiz Document project read from a CSV file with column names: 'SourceEmailAddress','DestinationEmailAddress'.
...
Function Select-MSPC_Workgroup {
    ...
    #######################################
    # Display all Document workgroups
    #######################################
   ...
   do {
    ...
    #######################################
    # Prompt for the Document Workgroup
    #######################################
...
Function Select-MSPC_Customer {
    ...
    #######################################
    # Display all Document customers
    #######################################
    ...
    #######################################
    # {Prompt for the Document customer
    #######################################
...
### Function to display all Document connectors
...
Write-Host $msg
    ...
    Write-Host
    Write-Host -Object  "INFO: Retrieving Document connectors ..."
    
    do {
        $connectorsPage = @(Get-MW_MailboxConnector -ticket $script:mwTicket -OrganizationId $CustomerOrganizationId -PageOffset $connectorOffSet -PageSize $connectorPageSize | where {$_.ProjectType -eq 'Storage' -and $_.ExportType -eq 'SharePoint' -and ($_.ImportType -eq 'SharePointOnlineAPI' -or $_.ImportType -eq 'SharePointBeta')} | sort ProjectType,Name )
    ...
    if($script:connectors -ne $null -and $script:connectors.Length -ge 1) {
        Write-Host -ForegroundColor Green -Object ("SUCCESS: "+ $script:connectors.Length.ToString() + " Document connector(s) found.") 
    }
    else {
        Write-Host -ForegroundColor Red -Object  "INFO: No Document connectors found." 
        Exit
    }
    ...
    if($script:connectors -ne $null) {       
        ...
        Write-Host -ForegroundColor Yellow -Object "ACTION: Select the Document connector:" 
...
Function Import-MW_DocumentUserMapping {
    param 
    (      
    ...
    Write-Host                                                                   
    Write-Host "INFO: Adding UserMapping(s) to the Document MigrationWiz project '$($connector.Name)'"
    Write-Host 

        # add items to a MigrationWiz only user Document project
        do {
        $confirm = (Read-Host -prompt "Would you like to import Document UserMapping from a CSV file with 'SourceEmailAddress','DestinationEmailAddress' columns?  [Y]es or [N]o")
            if($confirm -eq "Y") {            
                $UserMapping = $true
        ...
        if($script:inputFile -eq "") { 
            ...
            if($users) {
                Write-Host -ForegroundColor Green  "SUCCESS: $script:inputFile CSV file imported with $($users.Count) UserMapping." 
    ...
    if($UserMapping) {
        ...
        Write-Host
        Write-Host "INFO: Applying $($users.Count) UserMapping added to MigrationWiz Document project."
        ...
        foreach($user in $users) {
            ...
            if($AdvancedOptionsString -notmatch $FullString ) {
                ...
                $alreadyExistCount+=1
                Write-Host -ForegroundColor Yellow "WARNING: $FullString already exists. UserMapping skipped." 
            }
        }

        #Adding UserMappings into Advancedoptions   
        Try {
            ...
            Write-Host
            Write-Host -ForegroundColor Green "SUCCESS: $count UserMapping added to MigrationWiz Document project."

            if($alreadyExistCount -gt 0){
                Write-Host
                Write-Host -ForegroundColor Yellow "WARNING: $alreadyExistCount UserMapping were not added to MigrationWiz Document project because they already existed." 
            }
        }
        Catch {
            Write-Host -ForegroundColor Red "ERROR: Failed to add UserMapping added to MigrationWiz Document project advanced options."
            Write-Host -ForegroundColor Red $_.Exception.Message
            Exit
        }            
    }
...
#Log file
$logFileName = "$(Get-Date -Format yyyyMMdd)_Add-MWDocumentUserMappings.log"
...
write-host 
$msg = "####################################################################################################`
                         ADD DOCUMENT USER MAPPINGS                   `
####################################################################################################"
Write-Host $msg
Log-Write -Message "DOCUMENT USER MAPPING IMPORT" 

Write-Host
Import-MW_DocumentUserMapping $script:connector
